...
class TestCCTrades:
    ...
    async def test_cc_trade(self, wallets_prefarm):
        ...
        assert offer["littlelambocoin"] == -10
    ...
    async def test_cc_trade_with_multiple_colours(self, wallets_prefarm):
        ...
        assert success is True
        assert offer is not None
        assert offer["littlelambocoin"] == -1000
    ...
    async def test_cc_trade_cancel_insecure(self, wallets_prefarm):
        ...
        spendable_littlelambocoin = await wallet_a.get_spendable_balance()

        offer_dict = {1: 10, 2: -30, 3: 30}

        success, trade_offer, error = await trade_manager_a.create_offer_for_ids(offer_dict, file)

        spendable_littlelambocoin_after = await wallet_a.get_spendable_balance()
        ...
        assert spendable_littlelambocoin == spendable_littlelambocoin_after + locked_sum
        assert success is True
        assert trade_offer is not None
        ...
        assert spendable_littlelambocoin == spendable_after_cancel_1
    ...
    async def test_cc_trade_cancel_secure(self, wallets_prefarm):
        ...
        await time_out_assert(15, wallet_a.get_spendable_balance, spendable_littlelambocoin)
