...
async def add_dummy_connection(server: HDDcoinServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        server.littlelambocoin_ca_crt_path.read_bytes(), server.littlelambocoin_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.littlelambocoin_ca_crt_path, server.littlelambocoin_ca_key_path, dummy_crt_path, dummy_key_path
    )
