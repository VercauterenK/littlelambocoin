...
- We added a simple profiler to track performance times in the application (see /littlelambocoin/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `littlelambocoin configure -t true|false`).
...
- Secret wallet key is hidden by default in `littlelambocoin keys show`. Use `littlelambocoin keys show --show-mnemonic-seed` for private keys.
...
- This is the 1.0 release of HDDcoinlisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [littlelambocoinlisp.com](https://littlelambocoinlisp.com) shortly.
...
- The `src` directory in littlelambocoin-blockchain has been changed to `littlelambocoin` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `littlelambocoin/`
...
- You should review the [release notes for v1.0.2](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/littlelambocoin-blockchain).
...
- @jespino changed `littlelambocoin keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `littlelambocoin keys show`.
...
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `littlelambocoin keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/littlelambocoin-blockchain/).
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/littlelambocoin-blockchain).
...
- This is a hard fork/breaking change from RC6/7. THDD Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.littlelambocoin.org/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `littlelambocoin farm summary` will now display THDD or HDD as appropriate.
...
- `littlelambocoin farm summary` displayed some incorrect amounts.
...
- `littlelambocoin netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `littlelambocoin netspace`. Thanks @altendky.
- There was a typo in `littlelambocoin plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/issues/1189)
...
- Remove `littlelambocoin plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `littlelambocoin keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- Generators have been re-worked in CLVM. We added a littlelambocoinlisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during HDDcoinLisp native deserialization.
...
- HDDcoin wheels not on Pypi and some dependecies not found there also are now on pypi.littlelambocoin.org.
...
- `littlelambocoin farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `littlelambocoin wallet` had various bugs.
...
- RC3 is a new chain to support the last major littlelambocoinlisp changes. THDD from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `littlelambocoin -h` and `littlelambocoin SUBCOMMAND -h` can be your guide.
...
- `littlelambocoin plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `littlelambocoin netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `littlelambocoin init` only migrates release candidate directories. The versioned sub directories under `~/littlelambocoin` will be going away before mainnet.
...
- `littlelambocoin configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/releases/tag/1.0rc1) was well.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [HDDcoin Blockchain GUI](https://crowdin.com/project/littlelambocoin-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/HDDcoin-Network/littlelambocoin-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `littlelambocoin plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'littlelambocoin wallet send' now takes in THDD or HDD as units instead of mojos.
...
- You can now send and receive transactions with the command line. Try `littlelambocoin wallet -h` to learn more. Also, `littlelambocoin wallet` now requires a third argument of `show`, therefor you will use `littlelambocoin wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [littlelambocoin blockchain gui](https://crowdin.com/project/littlelambocoin-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `littlelambocoin init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `littlelambocoin init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- The GUI is now in a separate submodule repository from littlelambocoin-blockchain, [littlelambocoin-blockchain-gui](https://github.com/HDDcoin-Network/littlelambocoin-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `littlelambocoin-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `littlelambocoin wallet`, was incorrectly displaying THDD balances as if they were Coloured Coins.
...
- `littlelambocoin plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `littlelambocoin plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- `littlelambocoin show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `littlelambocoin show` handles sub block 0 better.
...
- `littlelambocoin plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `littlelambocoin plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `littlelambocoin show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `littlelambocoin configure --enable-upnp`.
...
- `littlelambocoin netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `littlelambocoin show` of the GUI use `littlelambocoin netspace -d 1000` as `littlelambocoin netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [littlelambocoin.org](https://littlelambocoin.org/). Among the improvements this gives the HDDcoin blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- There is a new cli command - `littlelambocoin configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `littlelambocoin configure -h`. Note that if littlelambocoin services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- Tests now place a cache of blocks and plots in the ~/.littlelambocoin/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run littlelambocoin-blockchain.
...
- `littlelambocoin show -s` now is less brain dead when a node is initially starting to sync.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install littlelambocoin-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/tree/dev/electron-react/src/locales) of the littlelambocoin-blockchain repository.
...
- The rpc interfaces of all littlelambocoin services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test littlelambocoin since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve littlelambocoin-blocklittlelambocoinn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from HDDcoin Network at backup.littlelambocoin.org. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the HDDcoin backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.littlelambocoin.org, your own installation, or a third party's version of it.
...
- `littlelambocoin init` now automatically discovers previous releases in each new release.

### Fixed

- `littlelambocoin show -w` should now more reliably work. Wallet balances should be more often correct.
...
- `littlelambocoin show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- See wallet balances in command line: `littlelambocoin show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `littlelambocoin keys sign` and `littlelambocoin keys verify` to allow farmers to certify their ownership of keys.
...
- `littlelambocoin start wallet-server` changed to `littlelambocoin start wallet`, for consistency.
...
- Removed legacy scripts such as littlelambocoin-stop-server, littlelambocoin-restart-harvester, etc.
...
"littlelambocoin plots create" and "littlelambocoin plots check".
- Add plot directories to config.yaml from the cli with "littlelambocoin plots add".
...
- Starting littlelambocoin now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'littlelambocoin keys' command line.
...
- We added a `littlelambocoin keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the littlelambocoin-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `littlelambocoin netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `littlelambocoin-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `littlelambocoin-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `littlelambocoin-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `littlelambocoin-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `littlelambocoin init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `littlelambocoin start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `littlelambocoin-start-harvester`.
...
- The user interface now refers to littlelambocoin coins with their correct currency code of HDD.
...
- Now you can run estimates of the total space currently farming the network. Try `littlelambocoin netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during littlelambocoin init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `littlelambocoin show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `littlelambocoin show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/wiki) and placed in [INSTALL](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/wiki/Quick-Start-Guide)
...
- HDDcoin start commands like `littlelambocoin start farmer` and `littlelambocoin stop node` now keep track of process IDs in a run/ directory in your configuration directory. `littlelambocoin stop` is unlikely to work on Windows native for now. If `littlelambocoin start -r node` doesn't work you can force the run/ directory to be reset with `littlelambocoin start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `littlelambocoin start wallet-gui` has been chaned to `littlelambocoin start wallet` which launches but the GUI and server on MacOS and Linux. `littlelambocoin start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `littlelambocoin stop all` command sometimes fails, use `littlelambocoin-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `littlelambocoin init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `littlelambocoin-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/wiki/Updating-beta-software). If you've set `$LITTLELAMBOCOIN_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `littlelambocoin show`.

### Changed

- `littlelambocoin-restart-harvester` has been renamed from `littlelambocoin-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.littlelambocoin/beta-1.0b3/config/plots.yaml` after migrating
...
- We have revamped the littlelambocoin management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `littlelambocoin-start-farmer &`. The [README.md](https://github.com/HDDcoin-Network/littlelambocoin-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- HDDcoin fullnode, farmer and harvester now default to logging to littlelambocoin.log in the littlelambocoin-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- Added color logging
- Now littlelambocoin_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to littlelambocoin_full node, littlelambocoin_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new littlelambocoin services were started.
...
[unreleased]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/HDDcoin-Network/littlelambocoin-blockchain/releases/tag/Alpha-1.0
