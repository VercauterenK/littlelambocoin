...
def get_littlelambocoin_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "littlelambocoin_ca.crt")
    key = pkg_resources.resource_string(__name__, "littlelambocoin_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "LittleLamboCoin"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("littlelambocoin.org")]),
            critical=False,
        )
...
def main():
    return make_ca_cert(Path("./littlelambocoin_ca.crt"), Path("./littlelambocoin_ca.key"))
