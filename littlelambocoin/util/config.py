...
def create_default_littlelambocoin_config(root_path: Path, filenames=["config.yaml"]) -> None:
...
def load_config(
    ...
    if not path.is_file():
        ...
        print("** please run `littlelambocoin init` to migrate or create new config files **")
        # TODO: fix this hack
        sys.exit(-1)
