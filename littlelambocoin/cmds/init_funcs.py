...
from littlelambocoin.ssl.create_ssl import generate_ca_signed_cert, get_littlelambocoin_ca_crt_key, make_ca_cert
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'littlelambocoin keys generate'")
        return None
...
def create_all_ssl(root: Path):
    ...
    littlelambocoin_ca_crt_path.write_bytes(littlelambocoin_ca_crt)
    littlelambocoin_ca_key_path.write_bytes(littlelambocoin_ca_key)
    ...
    littlelambocoin_ca_crt, littlelambocoin_ca_key = get_littlelambocoin_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, littlelambocoin_ca_crt, littlelambocoin_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if littlelambocoin_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return littlelambocoin_init(root_path)


def littlelambocoin_version_number() -> Tuple[str, str, str, str]:
...
def littlelambocoin_init(root_path: Path):
    if os.environ.get("LITTLELAMBOCOIN_ROOT", None) is not None:
        print(
            f"warning, your LITTLELAMBOCOIN_ROOT is set to {os.environ['LITTLELAMBOCOIN_ROOT']}. "
            f"Please unset the environment variable and run littlelambocoin init again\n"
