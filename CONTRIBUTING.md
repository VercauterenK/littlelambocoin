...
This repo includes the code for the HDDcoin full node, farmer, and timelord (in littlelambocoin folder), which are all written in python.
...
. ./activate
pip install ".[dev]"
black littlelambocoin tests && mypy littlelambocoin tests && flake8 littlelambocoin tests
...
6. Install these linters https://github.com/HDDcoin-Network/littlelambocoin-blockchain/tree/main/.github/linters

## Testnets and review environments

With the launch of `1.0.0` we will begin running an official `testnet`.  
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the littlelambocoin blockchain.
